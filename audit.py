...
def fetch_hashtag_stats(hashtags):
    ...
    for p in raw:
        ...
        grouped[tag].append({
            ...
            "likes": max(0, p.get("likesCount", 0) or 0),
...
from analysis import (split_own_vs_collab, views_metrics,
                      analyze_scope, signal_line)
...
def parse_post(p):
    ...
    raw_likes = p.get("likesCount", 0) or 0
    caption_full = p.get("caption") or ""
    return {
        "shortcode": p.get("shortCode") or p.get("shortcode"),
        "format": format_str,
        "caption": caption_full[:140],
        "caption_full": caption_full[:700],
        "owner": p.get("ownerUsername") or "",
        "hashtags": p.get("hashtags") or [],
        "product_type": p.get("productType") or "",
        "likes": max(0, raw_likes),
        "likes_hidden": raw_likes < 0,
        "comments": p.get("commentsCount", 0) or 0,
...
def build_window_data(raw_profiles, window_days):
    ...
    for profile in raw_profiles:
        ...
        # Collab post = muncul di grid tapi ownerUsername akun lain.
        # Itu audiens pinjaman, dikeluarkan dari skor akun.
        own_posts, collab_posts = split_own_vs_collab(in_window, username)
        for _cp in collab_posts:
            _cp["is_collab"] = True
        _vm = views_metrics(own_posts, follower_count)
        eng_total = sum(p["likes"] + p["comments"] for p in own_posts)
        ...
        out.append({
            ...
            "posts_window": own_posts,
            "eng_window": eng_total,
            "er": er,
            "reel_views": _vm["reel_views"],
            "reels_count": _vm["reels_count"],
            "views_per_reel": _vm["views_per_reel"],
            "reach_multiplier": _vm["reach_multiplier"],
            "collab_window": collab_posts,
            "collab_eng": sum(p["likes"] + p["comments"] for p in collab_posts),
            "collab_views": sum(p.get("views", 0) or 0 for p in collab_posts),
        })
    return out
...
def build_discord_summary(weekly, monthly):
    ...
    _nc = sum(1 for s in ROSTER.values() if s == "corporate")
    _na = sum(1 for s in ROSTER.values() if s == "agent")
    _nk = sum(1 for s in ROSTER.values() if s == "competitor")
    lines.append(f"{len(ROSTER)} akun ({_nc} corp · {_na} agent · {_nk} competitor)")
    lines.append("")
...
def build_pdf(data, top3, window_days, label, out_path,
    ...
    story.append(Paragraph(
        ...
        f"({window_days} hari) · {len(ROSTER)} akun · automated", small))
    story.append(Spacer(1, 8))

    # Scoreboard
    story.append(Paragraph("Scoreboard", h2))
    rows = [["Akun", "Status", "Followers", f"Posts {window_days}d", f"Eng. {window_days}d", "ER %", "Reel Views", "Views/Reel"]]
    ...
    for d in sorted_data:
        ...
        rows.append([
            ...
            Paragraph(f"{d.get('reel_views', 0):,}", tcell),
            Paragraph(f"{d.get('views_per_reel', 0):,.0f}", tcell),
        ])
    t = Table([[Paragraph(x, thdr) if i == 0 and isinstance(x, str) else x for x in r]
               for i, r in enumerate(rows)],
              colWidths=[3.9 * cm, 2.5 * cm, 2.2 * cm, 1.7 * cm, 1.8 * cm, 1.6 * cm, 2.3 * cm, 2.2 * cm])
    ...
    # Top 3 + analisa (cara / kenapa / tiru)
    def _esc(s):
        return (str(s).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;"))

    def _render_top(title, items):
        if not items:
            return
        ...
        for i, p in enumerate(items, 1):
            ...
            story.append(Paragraph(_esc(p["caption"]), small))
            if p.get("signals"):
                story.append(Paragraph(
                    f"<font color='{MUTED.hexval()}'>{_esc(signal_line(p))}</font>", small))
            for ln in (p.get("explain") or "").split("\n"):
                if ln.strip():
                    story.append(Paragraph(_esc(ln), small))
            story.append(Spacer(1, 7))

    if analysis_blocks:
        for _title, _items in analysis_blocks:
            _render_top(_title, _items)
    else:
        _render_top(f"🏆 Top 3 Posts {window_days}d", top3)
...
def main():
    ...
    # --- Analisa Top 3: dipisah milik-sendiri vs kompetitor ---
    _own_w = [d for d in w_data if d["status"] in ("corporate", "agent")]
    _comp_w = [d for d in w_data if d["status"] == "competitor"]
    _own_m = [d for d in m_data if d["status"] in ("corporate", "agent")]
    _comp_m = [d for d in m_data if d["status"] == "competitor"]
    w_blocks = [
        ("🏆 Top 3 Post KITA (7d) — corporate + agent",
         analyze_scope(_own_w, "akun milik sendiri (corporate + agent)", 7)),
        ("🔴 Top 3 Post KOMPETITOR (7d)",
         analyze_scope(_comp_w, "akun kompetitor", 7)),
    ]
    m_blocks = [
        ("🏆 Top 3 Post KITA (30d) — corporate + agent",
         analyze_scope(_own_m, "akun milik sendiri (corporate + agent)", 30)),
        ("🔴 Top 3 Post KOMPETITOR (30d)",
         analyze_scope(_comp_m, "akun kompetitor", 30)),
    ]

    # --- Views + collab + top post ke ringkasan Discord ---
    _tv = sorted(m_data, key=lambda d: d.get("reel_views", 0), reverse=True)[:3]
    if any(d.get("reel_views") for d in _tv):
        summary += "\n\n**👁 REEL VIEWS 30d** (reel-only — carousel/foto tidak punya data views)"
        for d in _tv:
            summary += (f"\n@{d['username']} — {d.get('reel_views', 0):,} views / "
                        f"{d.get('reels_count', 0)} reel · {d.get('views_per_reel', 0):,.0f}/reel "
                        f"· reach {d.get('reach_multiplier', 0):.1f}x follower")
    _cb = [d for d in m_data if d.get("collab_window")]
    if _cb:
        summary += ("\n⚠ Collab post dikeluarkan dari skor: "
                    + ", ".join(f"@{d['username']} ({len(d['collab_window'])})" for d in _cb[:5]))
    if w_blocks[0][1]:
        _t = w_blocks[0][1][0]
        summary += f"\n\n🏆 Post kita terbaik 7d: @{_t['account']} — {signal_line(_t)}"
    ...
    for label, dat in [("weekly", w_data), ("monthly", m_data)]:
        ...
        with open(sp, "w") as f:
            json.dump({
                ...
                "top_analysis": [
                    {"blok": _t2, "posts": _i2}
                    for _t2, _i2 in (w_blocks if label == "weekly" else m_blocks)
                ],
    ...
    post_discord_combined(summary[:1950], [w_pdf, m_pdf])
